# Rotational Grazing Planner | RanchAssist™

> Map paddocks, estimate available forage, arrange moves, and build a projected graze/rest schedule.

**Live app:** [https://rotationalgrazingplanner.ranchassist.com](https://rotationalgrazingplanner.ranchassist.com)  
...
A temporary map-based grazing planner that connects paddock geometry and forage assumptions with herd demand, rotation order, target recovery periods, and projected entry/exit dates.

Rotational Grazing Planner is one of the standalone tools in the RanchAssist ecosystem. The product is designed around a focused workflow: open the tool for the ranch job in front of you, enter or map the information that matters, review assumptions and calculations, and leave with a result that can be saved, exported, printed, copied, or shared.
...
- Draw/edit named paddock polygons over satellite imagery.
- Add water points, gates, temporary fence lines, and a herd start marker.
- Store per-paddock acres, forage, utilization, target residual/reserve, recovery target, availability, and notes.
- Support Equal Days, Capacity-Based, Target Rest-Period, and Manual planning modes.
- Drag/reorder the paddock sequence.
- Calculate projected graze days and entry/exit dates from a chosen start date.
- Warn when a planned return occurs before the user-defined recovery target.
- Show paddock capacity bars, map sequence styling, and a rotation timeline/calendar.
- Adjust temporary scenarios for forage, herd size, utilization, recovery target, or a user-defined drought/conservative factor.

## Typical workflow

1. Map or open the paddocks.
2. Define herd size/class/weight and intake assumptions.
3. Enter forage and recovery assumptions for each paddock.
4. Choose a planning mode and rotation sequence.
5. Select a start date.
6. Review graze days, cycle length, rest intervals, warnings, and timeline.
7. Save the `.ra` project or export the map/schedule.

## Calculation / measurement model

- Available forage per paddock is based on paddock acreage × forage dry matter/acre × user-entered utilization/reserve assumptions.
- Herd daily forage demand is derived from head count, average weight, and intake percentage.
- Capacity-based graze days = available paddock forage ÷ herd daily demand.
- Projected entry/exit dates follow the chosen sequence, start date, and calculated or manual graze duration.
- Projected rest interval is compared with the user-defined recovery target and warnings are shown when the target is not met.
...
A saved RanchAssist `.ra` file for this tool should preserve enough information to resume meaningful editing, including herd inputs, paddock geometry/data, rotation sequence, planning mode, start date, recovery targets, scenarios, notes, map camera, markers, temporary fence lines, and layer state.
...
Mapbox is required for satellite mapping and paddock drawing. Configure `MAPBOX_ACCESS_TOKEN` in Apps Script Script Properties using a browser-safe public token. Email sharing uses Apps Script `MailApp`.
...
Forage availability and recovery are site-specific and change with weather, season, soil, plant community, prior use, and management. The planner does not provide an agronomic guarantee or forecast; all recovery targets and scenario adjustments remain user-defined.
